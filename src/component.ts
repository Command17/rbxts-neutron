...
/**
 * Base class for class components. All components should
 * extend from this class.
 *
 * ```ts
...
/**
 * Wraps a ClassComponent to a BaseComponent
...
export function ClassComponent<T extends Instance>(config: ComponentConfig) {
	return <B extends new (instance: T, tag: string) => BaseClassComponent<T>>(componentClass: B) => {
		...
		const runner = new ComponentRunner(config, wrappedComponentClass);

		componentClassToRunner.set(wrappedComponentClass, runner);
